...
void* gumbo_browser_malloc(void*, size_t size)
{
	return std::malloc(size);
}

void gumbo_browser_noop_free(void*, void*)
{
}

GumboOptions gumbo_browser_options()
{
	GumboOptions options = kGumboDefaultOptions;
	options.allocator = &gumbo_browser_malloc;
	options.deallocator = &gumbo_browser_noop_free;
	...
	const string& user_styles )
{
	...
	// XJ380 userland currently hits invalid frees while tearing down Gumbo's parse
	// tree. Leak the temporary tree instead of crashing the browser process.
#ifndef LITEHTML_KEEP_GUMBO_OUTPUT
...
GumboOutput* document::parse_html(estring str)
{
	...
	GumboOptions gumbo_options = gumbo_browser_options();
	GumboOutput* output = gumbo_parse_with_options(&gumbo_options, m_text.data(), m_text.size());
	...
	if (meta_encoding != encoding::null)
	{
		...
		if (new_encoding != str.encoding)
		{
			...
			output = gumbo_parse_with_options(&gumbo_options, m_text.data(), m_text.size());
		}
	}
