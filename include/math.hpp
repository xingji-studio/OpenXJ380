#pragma once
#include <proto.hpp>
// 哦，我们亲爱的派
#define PI                                                                                                             \
    (float64_t)3.1415926535 // 搞笑版:3.14159265358979323846264338327950288419716939937510582097494459230781640628620899
#define TAU                                                                                                            \
    (float64_t)6.2831853071 // 搞笑版:6.28318530717958647692528676655900576839433879875021164194988918553642212019584998
#define PI_DIV_2                                                                                                       \
    (float64_t)1.5707963267 // 搞笑版:1.
#define LONG_LONG_MAX (0x7FFFFFFFFFFFFFFFLL)      // 最大的long long整数
#define LONG_LONG_MIN (-0x7FFFFFFFFFFFFFFFLL - 1) // 最小的long long整数
#define UINT64_MAX    (0xFFFFFFFFFFFFFFFFULL)     // 最大的unsigned long long整数
#define UINT64_MIN    (0x0000000000000000ULL)     // 最小的unsigned long long整数
#define INT64_MAX     (0x7FFFFFFFFFFFFFFFLL)      // 最大的signed long long整数
#define INT64_MIN     (-0x7FFFFFFFFFFFFFFFLL - 1) // 最小的signed long long整数
#define UINT32_MAX    (0xFFFFFFFFUL)              // 最大的unsigned int整数
#define UINT32_MIN    (0x00000000UL)              // 最小的unsigned int整数
#define INT32_MAX     (0x7FFFFFFFUL)              // 最大的signed int整数
#define INT32_MIN     (-0x7FFFFFFFUL - 1)         // 最小的signed int整数
#define UINT16_MAX    (0xFFFFU)                   // 最大的unsigned short整数
#define UINT16_MIN    (0x0000U)                   // 最小的unsigned short整数
#define INT16_MAX     (0x7FFF)                    // 最大的signed short整数
#define INT16_MIN     (-0x7FFF - 1)               // 最小的signed short整数
#define UINT8_MAX     (0xFFU)                     // 最大的unsigned char整数
#define UINT8_MIN     (0x00U)                     // 最小的unsigned char整数
#define INT8_MAX      (0x7F)                      // 最大的signed char整数
#define INT8_MIN      (-0x7F - 1)                 // 最小的signed char整数
#define SHRT_MAX      (0x7FFF)                    // 最大的short整数
#define SHRT_MIN      (-0x7FFF - 1)               // 最小的short整数
#define INT_MAX       (0x7FFFFFFF)                // 最大的int整数
#define INT_MIN       (-0x7FFFFFFF - 1)           // 最小的int整数
#define UINT_MAX      (0xFFFFFFFFU)               // 最大的unsigned int整数
#define UINT_MIN      (0x00000000U)               // 最小的unsigned int整数
#define LONG_MAX      (0x7FFFFFFFFFFFFFFFL)       // 最大的long整数
#define LONG_MIN      (-0x7FFFFFFFFFFFFFFFL - 1)  // 最小的long整数

#define FORCE_EVAL(x)                                                                                                  \
    do                                                                                                                 \
    {                                                                                                                  \
        if (sizeof(x) == sizeof(float))                                                                                \
        {                                                                                                              \
            volatile float __x __attribute__((unused));                                                                \
            __x = (x);                                                                                                 \
        }                                                                                                              \
        else if (sizeof(x) == sizeof(double))                                                                          \
        {                                                                                                              \
            volatile double __x __attribute__((unused));                                                               \
            __x = (x);                                                                                                 \
        }                                                                                                              \
        else                                                                                                           \
        {                                                                                                              \
            volatile long double __x __attribute__((unused));                                                          \
            __x = (x);                                                                                                 \
        }                                                                                                              \
    } while (0)
int round(float64_t x)
{
    if (x >= 0.0)
        return (int)(x + 0.5);
    else
        return (int)(x - 0.5);
}
int32_t equal(float64_t elem1, float64_t elem2)
{
    if ((elem1 - elem2 < 0.0000001) && (elem1 - elem2 > -0.0000001))
        return 1; // 相等
    else
        return 0; // 不相等
}
static const double rounders[10 + 1] = {
    0.5,          // 0
    0.05,         // 1
    0.005,        // 2
    0.0005,       // 3
    0.00005,      // 4
    0.000005,     // 5
    0.0000005,    // 6
    0.00000005,   // 7
    0.000000005,  // 8
    0.0000000005, // 9
    0.00000000005 // 10
};
char *ftoa(double f, char *buf, int precision)
{
    char *ptr = buf;
    char *p   = ptr;
    char *p1;
    char  c;
    long  intPart;

    if (precision > 10) precision = 10;
    if (f < 0)
    {
        f      = -f;
        *ptr++ = '-';
    }
    if (precision < 0)
    {
        if (f < 1.0)
            precision = 6;
        else if (f < 10.0)
            precision = 5;
        else if (f < 100.0)
            precision = 4;
        else if (f < 1000.0)
            precision = 3;
        else if (f < 10000.0)
            precision = 2;
        else if (f < 100000.0)
            precision = 1;
        else
            precision = 0;
    }
    if (precision) f += rounders[precision];
    intPart  = f;
    f       -= intPart;
    if (!intPart)
        *ptr++ = '0';
    else
    {
        p = ptr;
        while (intPart)
        {
            *p++     = '0' + intPart % 10;
            intPart /= 10;
        }
        p1 = p;
        while (p > ptr)
        {
            c      = *--p;
            *p     = *ptr;
            *ptr++ = c;
        }
        ptr = p1;
    }
    if (precision)
    {
        *ptr++ = '.';
        while (precision--)
        {
            f      *= 10.0;
            c       = f;
            *ptr++  = '0' + c;
            f      -= c;
        }
    }
    *ptr = 0;
    return ptr;
}

float ceilf(float x)
{
    float fract = x - (int)x; // 获取小数部分
    if (fract > 0)
    {
        return (int)x + 1; // 如果有小数部分，向上取整
    }
    else
    {
        return (int)x; // 如果没有小数部分，直接返回整数部分
    }
}

float floorf(float x)
{
    float fract = x - (int)x; // 获取小数部分
    if (fract < 0)
    {
        return (int)x - 1; // 如果是负数且有小数部分，向下取整
    }
    else
    {
        return (int)x; // 如果是正数或没有小数部分，直接返回整数部分
    }
}

float roundf(float number)
{
    if (number < 0.0f) { return ceilf(number - 0.5f); }
    else { return floorf(number + 0.5f); }
}

double fabs(double x)
{
    if (x < 0) { return -x; }
    else { return x; }
}

double floor(double x)
{
    double fract = x - (int)x;
    if (fract < 0) { return (int)x - 1; }
    else { return (int)x; }
}

double ceil(double x)
{
    double fract = x - (int)x; // 获取小数部分
    if (fract > 0)
    {
        return (int)x + 1; // 如果有小数部分，向上取整
    }
    else
    {
        return (int)x; // 如果没有小数部分，直接返回整数部分
    }
}

double fmod(double x, double y)
{
    if (y == 0) { return __builtin_nanf(""); }
    double intPart   = x / y;
    double remainder = x - intPart * y;
    if (remainder < 0) { remainder += y; }
    else if (remainder > y) { remainder -= y; }
    return remainder;
}

double cos(double x)
{
    double sum  = 0.0;
    double term = 1.0;
    int    n    = 0;

    for (n = 0; term > 1e-15; n++)
    {
        term  = term * (-1) * (2 * n) * (2 * n - 1) / ((2 * n) * (2 * n - 1));
        sum  += term;
    }

    return sum;
}
double sqrt(double number)
{
    if (number < 0) { return __builtin_nanf(""); }

    double x       = number;
    double epsilon = 1e-15;
    double diff;

    do
    {
        x    = (x + number / x) / 2;
        diff = fabs(x - number / x);
    } while (diff > epsilon);

    return x;
}
double acos(double x)
{
    double x0 = x;
    double x1;
    double tolerance      = 1e-15;
    double max_iterations = 1000;
    int    iterations     = 0;

    while (iterations < max_iterations)
    {
        x1 = x0 - (-1 / sqrt(1 - x0 * x0)) / (1 / cos(x0));
        if (fabs(x1 - x0) < tolerance) { break; }
        x0 = x1;
        iterations++;
    }

    return x1;
}

double pow(double x, int y)
{
    double result = 1.0;
    for (int i = 0; i < y; i++)
    {
        result *= x;
    }
    return result;
}
double ldexp(double x, int exp)
{
    int n = 2;
    for (int i = 0; i < exp - 1; i++)
    {
        n *= 2;
    }
    return x * (double)(n);
}

int abs(int x)
{
    return (x < 0 ? -x : x);
}